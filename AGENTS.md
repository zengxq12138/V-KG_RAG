...
VKGRAG is an advanced Retrieval-Augmented Generation (RAG) framework designed to enhance information retrieval and generation through graph-based knowledge representation.

## Project Structure & Module Organization
- `vkgrag/`: Core Python package with orchestrators (`vkgrag/vkgrag.py`), storage adapters in `kg/`, LLM bindings in `llm/`, and helpers such as `operate.py` and `utils_*.py`.
- `vkgrag-api/`: FastAPI service (`vkgrag_server.py`) with routers under `routers/` and Gunicorn launcher `run_with_gunicorn.py`.
- `vkgrag_webui/`: React 19 + TypeScript client driven by Bun + Vite; UI components live in `src/`.
...
- `vkgrag-server` or `uvicorn vkgrag.api.vkgrag_server:app --reload`: start the API locally; ensure `.env` is present.
...
- Use `vkgrag.utils.logger` instead of `print`; respect logger configuration flags.
- Extend storage or pipeline abstractions via `vkgrag.base` and keep reusable helpers in the existing `utils_*.py`.
...
- Export required `VKGRAG_*` environment variables before running integration or storage tests.
...
- Configure storage backends through `VKGRAG_*` variables and validate them with `docker-compose` services when needed.
- Treat `vkgrag.log*` as local artefacts; purge sensitive information before sharing logs or outputs.
