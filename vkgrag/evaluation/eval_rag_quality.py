#!/usr/bin/env python3
"""
RAGAS Evaluation Script for VKGRAG System
...
Usage:
    ...
    python vkgrag/evaluation/eval_rag_quality.py -d my_test.json

    # Specify custom RAG endpoint
    python vkgrag/evaluation/eval_rag_quality.py --ragendpoint http://my-server.com:9621
    python vkgrag/evaluation/eval_rag_quality.py -r http://my-server.com:9621
...
Results are saved to: vkgrag/evaluation/results/
...
from vkgrag.utils import logger
...
class RAGEvaluator:
    ...
    def __init__(self, test_dataset_path: str = None, rag_api_url: str = None):
        ...
        Args:
            ...
            rag_api_url: Base URL of VKGRAG API (e.g., http://localhost:9621)
        ...
        if rag_api_url is None:
            rag_api_url = os.getenv("VKGRAG_API_URL", "http://localhost:9621")
    ...
    def _display_configuration(self):
        ...
        logger.info("  • VKGRAG API:         %s", self.rag_api_url)
    ...
    async def generate_rag_response(
        ...
        """
        Generate RAG response by calling VKGRAG API.
        ...
        Raises:
            Exception: If VKGRAG API is unavailable.
        """
        try:
            payload = {
                "query": question,
            ...
            api_key = os.getenv("VKGRAG_API_KEY")
            ...
            raise Exception(
                f"❌ Cannot connect to VKGRAG API at {self.rag_api_url}\n"
                f"   Make sure VKGRAG server is running:\n"
                f"   python -m vkgrag.api.vkgrag_server\n"
                f"   Error: {str(e)}"
            )
        except httpx.HTTPStatusError as e:
            raise Exception(
                f"VKGRAG API error {e.response.status_code}: {e.response.text}"
            )
            ...
            raise Exception(f"Error calling VKGRAG API: {type(e).__name__}: {str(e)}")
    ...
    async def evaluate_responses(self) -> List[Dict[str, Any]]:
        ...
        logger.info("%s", "=" * 70)
        logger.info("🚀 Starting RAGAS Evaluation of VKGRAG System")
...
async def main():
    ...
    Command-line arguments:
        ...
        --ragendpoint, -r: VKGRAG API endpoint URL (default: http://localhost:9621 or $VKGRAG_API_URL)
    ...
    try:
        ...
        parser = argparse.ArgumentParser(
            description="RAGAS Evaluation Script for VKGRAG System",
...
Examples:
  ...
  python vkgrag/evaluation/eval_rag_quality.py -d my_test.json -r http://localhost:9621
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="VKGRAG API endpoint URL (default: http://localhost:9621 or $VKGRAG_API_URL environment variable)",
        )

        args = parser.parse_args()

        logger.info("%s", "=" * 70)
        logger.info("🔍 RAGAS Evaluation - Using Real VKGRAG API")
        logger.info("%s", "=" * 70)
