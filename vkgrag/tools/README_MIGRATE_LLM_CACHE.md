...
This tool migrates VKGRAG's LLM response cache between different KV storage implementations. It specifically migrates caches generated during file extraction (mode `default`), including entity extraction and summary caches.
...
The LLM Cache Migration Tool reads the storage configuration of the VKGRAG Server and provides an LLM migration option to select source and destination storage. Ensure that both the source and destination storage have been correctly configured and are accessible via the VKGRAG Server before cache migration.

## Usage

### Basic Usage

Run from the VKGRAG project root directory:

```bash
python -m vkgrag.tools.migrate_llm_cache
# or
python vkgrag/tools/migrate_llm_cache.py
```
