...
from vkgrag.llm.binding_options import (
...
from vkgrag.base import OllamaServerInfos
import sys
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="VKGRAG API Server")
    ...
    parser.add_argument(
        "--key",
        type=str,
        default=get_env_value("VKGRAG_API_KEY", None),
        help="API key for authentication. This protects vkgrag server against unauthorized access",
    )
    ...
    parser.add_argument(
        ...
        help="Name for the simulated Ollama model (default: from env or vkgrag)",
    )
    ...
    args.kv_storage = get_env_value(
        "VKGRAG_KV_STORAGE", DefaultRAGStorageConfig.KV_STORAGE
    )
    args.doc_status_storage = get_env_value(
        "VKGRAG_DOC_STATUS_STORAGE", DefaultRAGStorageConfig.DOC_STATUS_STORAGE
    )
    args.graph_storage = get_env_value(
        "VKGRAG_GRAPH_STORAGE", DefaultRAGStorageConfig.GRAPH_STORAGE
    )
    args.vector_storage = get_env_value(
        "VKGRAG_VECTOR_STORAGE", DefaultRAGStorageConfig.VECTOR_STORAGE
    )
    ...
    args.token_secret = get_env_value("TOKEN_SECRET", "vkgrag-jwt-default-secret")
    ...
    ollama_server_infos.VKGRAG_NAME = args.simulated_model_name
    ollama_server_infos.VKGRAG_TAG = args.simulated_model_tag
...
def initialize_config(args=None, force=False):
    ...
    which is useful for programmatic usage, testing, or embedding VKGRAG
