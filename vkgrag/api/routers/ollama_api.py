...
from vkgrag.utils import TiktokenTokenizer
from vkgrag.api.utils_api import get_combined_auth_dependency
from fastapi import Depends


# query mode according to query prefix (bypass is not VKGRAG quer mode)
...
class OllamaAPI:
    def __init__(self, rag: VKGRAG, top_k: int = 60, api_key: Optional[str] = None):
        self.rag = rag
    ...
    def setup_routes(self):
        ...
        async def get_tags():
            ...
            return OllamaTagResponse(
                models=[
                    {
                        "name": self.ollama_server_infos.VKGRAG_MODEL,
                        "model": self.ollama_server_infos.VKGRAG_MODEL,
                        "modified_at": self.ollama_server_infos.VKGRAG_CREATED_AT,
                        "size": self.ollama_server_infos.VKGRAG_SIZE,
                        "digest": self.ollama_server_infos.VKGRAG_DIGEST,
                        "details": {
                            "parent_model": "",
                            "format": "gguf",
                            "family": self.ollama_server_infos.VKGRAG_NAME,
                            "families": [self.ollama_server_infos.VKGRAG_NAME],
                            "parameter_size": "13B",
                ...
                models=[
                    {
                        ...
                        "size_vram": self.ollama_server_infos.VKGRAG_SIZE,
                    }
                ]
            )
        ...
        async def generate(raw_request: Request):
            ...
            For compatibility purpose, the request is not processed by VKGRAG,
                    ...
                    async def stream_generator():
                        try:
                            ...
                            if isinstance(response, str):
                                ...
                                data = {
                                    "model": self.ollama_server_infos.VKGRAG_MODEL,
                                    "created_at": self.ollama_server_infos.VKGRAG_CREATED_AT,
                                    "response": response,
                                    "done": False,
                                }
        ...
        async def chat(raw_request: Request):
            ...
            Routes user queries through VKGRAG by selecting query mode based on query prefix.
