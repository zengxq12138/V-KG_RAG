...
from vkgrag.utils import setup_logger, get_env_value
...
errorlog = os.getenv("ERROR_LOG", log_file_path)  # Default write to vkgrag.log
accesslog = os.getenv("ACCESS_LOG", log_file_path)  # Default write to vkgrag.log
...
def on_starting(server):
    ...
    # Log the location of the VKGRAG log file
    print(f"VKGRAG log file: {log_file_path}\n")
...
def post_fork(server, worker):
    ...
    setup_logger("vkgrag", log_level, add_filter=True, log_file_path=log_file_path)

    # Set up vkgrag submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith("vkgrag."):
