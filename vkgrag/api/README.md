# VKGRAG Server and WebUI

The VKGRAG Server is designed to provide a Web UI and API support. The Web UI facilitates document indexing, knowledge graph exploration, and a simple RAG query interface. VKGRAG Server also provides an Ollama-compatible interface, aiming to emulate VKGRAG as an Ollama chat model. This allows AI chat bots, such as Open WebUI, to access VKGRAG easily.
...
### Before Starting VKGRAG Server

VKGRAG necessitates the integration of both an LLM (Large Language Model) and an Embedding Model to effectively execute document indexing and querying operations. Prior to the initial deployment of the VKGRAG server, it is essential to configure the settings for both the LLM and the Embedding Model. VKGRAG supports binding to various LLM/Embedding backends:
...
It is recommended to use environment variables to configure the VKGRAG Server. There is an example environment variable file named `env.example` in the root directory of the project. Please copy this file to the startup directory and rename it to `.env`. After that, you can modify the parameters related to the LLM and Embedding models in the `.env` file. It is important to note that the VKGRAG Server will load the environment variables from `.env` into the system environment variables each time it starts. **VKGRAG Server will prioritize the settings in the system environment variables to .env file**.
...
### Starting VKGRAG Server

The VKGRAG Server supports two operational modes:
...
```
vkgrag-gunicorn --workers 4
```

When starting VKGRAG, the current working directory must contain the `.env` configuration file. **It is intentionally designed that the `.env` file must be placed in the startup directory**. The purpose of this is to allow users to launch multiple VKGRAG instances simultaneously and configure different `.env` files for different instances. **After modifying the `.env` file, you need to reopen the terminal for the new settings to take effect.** This is because each time VKGRAG Server starts, it loads the environment variables from the `.env` file into the system environment variables, and system environment variables have higher precedence.
...
- `--workspace`: Workspace name, used to logically isolate data between multiple VKGRAG instances (default: empty)

### Launching VKGRAG Server with Docker

Using Docker Compose is the most convenient way to deploy and run the VKGRAG Server.

* Create a project directory.

* Copy the `docker-compose.yml` file from the VKGRAG repository into your project directory.
...
* Start the VKGRAG Server with the following command:
...
You can get the official docker compose file from here: [docker-compose.yml](https://raw.githubusercontent.com/HKUDS/VKGRAG/refs/heads/main/docker-compose.yml). For historical versions of VKGRAG docker images, visit this link: [VKGRAG Docker Images](https://github.com/HKUDS/VKGRAG/pkgs/container/vkgrag). For more details about docker deployment, please refer to [DockerDeployment.md](./../../docs/DockerDeployment.md).

### Offline Deployment

Official VKGRAG Docker images are fully compatible with offline or air-gapped environments. If you want to build up you own  offline enviroment, please refer to [Offline Deployment Guide](./../../docs/OfflineDeployment.md).

### Starting Multiple VKGRAG Instances

There are two ways to start multiple VKGRAG instances. The first way is to configure a completely independent working environment for each instance. This requires creating a separate working directory for each instance and placing a dedicated `.env` configuration file in that directory. The server listening ports in the configuration files of different instances cannot be the same. Then, you can start the service by running `vkgrag-server` in the working directory.

The second way is for all instances to share the same set of `.env` configuration files, and then use command-line arguments to specify different server listening ports and workspaces for each instance. You can start multiple VKGRAG instances in the same working directory with different command-line arguments. For example:

```
# Start instance 1
vkgrag-server --port 9621 --workspace space1

# Start instance 2
vkgrag-server --port 9622 --workspace space2
```
...
The VKGRAG Server can operate in the `Gunicorn + Uvicorn` preload mode. Gunicorn's multiple worker (multiprocess) capability prevents document indexing tasks from blocking RAG queries. Using CPU-exhaustive document extraction tools, such as docling, can lead to the entire system being blocked in pure Uvicorn mode.

Though VKGRAG Server uses one worker to process the document indexing pipeline, with the async task support of Uvicorn, multiple files can be processed in parallel. The bottleneck of document indexing speed mainly lies with the LLM. If your LLM supports high concurrency, you can accelerate document indexing by increasing the concurrency level of the LLM. Below are several environment variables related to concurrent processing, along with their default values:
...
### Install VKGRAG as a Linux Service

Create your service file `vkgrag.service` from the sample file: `vkgrag.service.example`. Modify the start options the service file:

```text
# Set Enviroment to your Python virtual enviroment
Environment="PATH=/home/netman/vkgrag-xyj/venv/bin"
WorkingDirectory=/home/netman/vkgrag-xyj
# ExecStart=/home/netman/vkgrag-xyj/venv/bin/vkgrag-server
ExecStart=/home/netman/vkgrag-xyj/venv/bin/vkgrag-gunicorn

```

> The ExecStart command must be either `vkgrag-gunicorn` or `vkgrag-server`; no wrapper scripts are allowed. This is because service termination requires the main process to be one of these two executables.

Install VKGRAG service. If your system is Ubuntu, the following commands will work:

```shell
sudo cp vkgrag.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl start vkgrag.service
sudo systemctl status vkgrag.service
sudo systemctl enable vkgrag.service
```

## Ollama Emulation

We provide Ollama-compatible interfaces for VKGRAG, aiming to emulate VKGRAG as an Ollama chat model. This allows AI chat frontends supporting Ollama, such as Open WebUI, to access VKGRAG easily.

### Connect Open WebUI to VKGRAG

After starting the vkgrag-server, you can add an Ollama-type connection in the Open WebUI admin panel. And then a model named `vkgrag:latest` will appear in Open WebUI's model management interface. Users can then send queries to VKGRAG through the chat interface. You should install VKGRAG as a service for this use case.
...
The default query mode is `hybrid` if you send a message (query) from the Ollama interface of VKGRAG. You can select query mode by sending a message with a query prefix.

A query prefix in the query string can determine which VKGRAG query mode is used to generate the response for the query. The supported prefixes include:
...
For example, the chat message `/mix What's VKGRAG?` will trigger a mix mode query for VKGRAG. A chat message without a query prefix will trigger a hybrid mode query by default.

`/bypass` is not a VKGRAG query mode; it will tell the API Server to pass the query directly to the underlying LLM, including the chat history. So the user can use the LLM to answer questions based on the chat history. If you are using Open WebUI as a front end, you can just switch the model to a normal LLM instead of using the `/bypass` prefix.

`/context` is also not a VKGRAG query mode; it will tell VKGRAG to return only the context information prepared for the LLM. You can check the context if it's what you want, or process the context by yourself.

### Add user prompt in chat

When using VKGRAG for content queries, avoid combining the search process with unrelated output processing, as this significantly impacts query effectiveness. User prompt is specifically designed to address this issue — it does not participate in the RAG retrieval phase, but rather guides the LLM on how to process the retrieved results after the query is completed. We can append square brackets to the query prefix to provide the LLM with the user prompt:
...
By default, the VKGRAG Server can be accessed without any authentication. We can configure the server with an API Key or account credentials to secure it.

* API Key:

```
VKGRAG_API_KEY=your-secure-api-key-here
...
The API key is passed using the request header `X-API-Key`. Below is an example of accessing the VKGRAG Server via API:
...
VKGRAG API Server implements JWT-based authentication using the HS256 algorithm. To enable secure access control, the following environment variables are required:
...
RESOURCE_GROUP_NAME=VKGRAG
LOCATION=swedencentral
RESOURCE_NAME=VKGRAG-OpenAI
...
## VKGRAG Server Configuration in Detail
...
VKGRAG supports binding to various LLM/Embedding backends:
...
```
vkgrag-server --llm-binding openai --help
vkgrag-server --llm-binding ollama --help
vkgrag-server --embedding-binding ollama --help
```
...
VKGRAG Server offers various storage implementations, with the default being an in-memory database that persists data to the WORKING_DIR directory. Additionally, VKGRAG supports a wide range of storage solutions including PostgreSQL, MongoDB, FAISS, Milvus, Qdrant, Neo4j, Memgraph, and Redis. For detailed information on supported storage options, please refer to the storage section in the README.md file located in the root directory.
...
```
VKGRAG_KV_STORAGE=PGKVStorage
VKGRAG_VECTOR_STORAGE=PGVectorStorage
VKGRAG_GRAPH_STORAGE=PGGraphStorage
VKGRAG_DOC_STATUS_STORAGE=PGDocStatusStorage
```

You cannot change storage implementation selection after adding documents to VKGRAG. Data migration from one storage implementation to another is not supported yet. For further information, please read the sample env file or config.ini file.

### LLM Cache Migration Between Storage Types

When switching the storage implementation in VKGRAG, the LLM cache can be migrated from the existing storage to the new one. Subsequently, when re-uploading files to the new storage, the pre-existing LLM cache will significantly accelerate file processing. For detailed instructions on using the LLM cache migration tool, please refer to[README_MIGRATE_LLM_CACHE.md](../tools/README_MIGRATE_LLM_CACHE.md)

### VKGRAG API Server Command Line Options
...
| --key                 | None          | API key for authentication. Protects the VKGRAG server against unauthorized access                                            |
...
Reranking query-recalled chunks can significantly enhance retrieval quality by re-ordering documents based on an optimized relevance scoring model. VKGRAG currently supports the following rerank providers:
...
```json
{
  "response": "VKGRAG is a graph-based RAG system...",
  "references": [
    {
      "reference_id": "1",
      "file_path": "/documents/intro.md",
      "content": [
        "VKGRAG is a retrieval-augmented generation system that combines knowledge graphs with vector similarity search...",
...
### For JWT Auth
# AUTH_ACCOUNTS='admin:admin123,user1:pass456'
# TOKEN_SECRET=your-key-for-VKGRAG-API-Server-xxx
# TOKEN_EXPIRE_HOURS=48

# VKGRAG_API_KEY=your-secure-api-key-here-123
# WHITELIST_PATHS=/api/*
# WHITELIST_PATHS=/health,/api/*
...
VKGRAG implements asynchronous document indexing to enable frontend monitoring and querying of document processing progress. Upon uploading files or inserting text through designated endpoints, a unique Track ID is returned to facilitate real-time progress monitoring.
