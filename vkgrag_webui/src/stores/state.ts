...
import { checkHealth, VKGRAGStatus } from '@/api/vkgrag'
...
const initAuthState = (): { isAuthenticated: boolean; isGuestMode: boolean; coreVersion: string | null; apiVersion: string | null; username: string | null; webuiTitle: string | null; webuiDescription: string | null } => {
  const token = localStorage.getItem('VKGRAG-API-TOKEN');
  const coreVersion = localStorage.getItem('VKGRAG-CORE-VERSION');
  const apiVersion = localStorage.getItem('VKGRAG-API-VERSION');
  const webuiTitle = localStorage.getItem('VKGRAG-WEBUI-TITLE');
  const webuiDescription = localStorage.getItem('VKGRAG-WEBUI-DESCRIPTION');
...
export const useAuthStore = create<AuthState>(set => {
  ...
  return {
    ...
    login: (token, isGuest = false, coreVersion = null, apiVersion = null, webuiTitle = null, webuiDescription = null) => {
      localStorage.setItem('VKGRAG-API-TOKEN', token);

      if (coreVersion) {
        localStorage.setItem('VKGRAG-CORE-VERSION', coreVersion);
      }
      if (apiVersion) {
        localStorage.setItem('VKGRAG-API-VERSION', apiVersion);
      }

      if (webuiTitle) {
        localStorage.setItem('VKGRAG-WEBUI-TITLE', webuiTitle);
      } else {
        localStorage.removeItem('VKGRAG-WEBUI-TITLE');
      }

      if (webuiDescription) {
        localStorage.setItem('VKGRAG-WEBUI-DESCRIPTION', webuiDescription);
      } else {
        localStorage.removeItem('VKGRAG-WEBUI-DESCRIPTION');
      }
    ...
    logout: () => {
      localStorage.removeItem('VKGRAG-API-TOKEN');
