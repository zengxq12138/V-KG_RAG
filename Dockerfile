...
COPY vkgrag_webui/ ./vkgrag_webui/
...
COPY --from=frontend-builder /app/vkgrag/api/webui ./vkgrag/api/webui
...
RUN mkdir -p /app/data/tiktoken \
    && uv run vkgrag-download-cache --cache-dir /app/data/tiktoken || status=$?; \
...
COPY --from=builder /app/vkgrag ./vkgrag
COPY pyproject.toml .
COPY setup.py .
COPY uv.lock .
...
ENTRYPOINT ["python", "-m", "vkgrag.api.vkgrag_server"]
